...
def define_components(mod):
    ...
    # Add new parameters to capture build years and expansion constraints
    mod.initial_build_year = Param(
        mod.TRANSMISSION_LINES,
        within=NonNegativeReals,  # Ensure the year is a non-negative number
        doc="The year when the transmission line was initially built."
    )
    mod.min_data_check(
        'trans_length_km', 'trans_efficiency', 'existing_trans_cap','initial_build_year')
    mod.trans_new_build_allowed = Param(
        mod.TRANSMISSION_LINES, within=Boolean, default=True)
    mod.trans_expand_after_year = Param(
        mod.TRANSMISSION_LINES,
        within=NonNegativeReals,
        default=lambda m, tx: m.initial_build_year[tx],
        doc="The year after which expansion of the transmission line is allowed."
    )
    mod.TRANS_BLD_YRS = Set(
        dimen=2,
        initialize=mod.TRANSMISSION_LINES * mod.PERIODS,
        filter=lambda m, tx, p: p >= m.trans_expand_after_year[tx] and m.trans_new_build_allowed[tx]
    )
    ...
    mod.TxCapacityNameplate = Expression(
        ...
        ) + (m.existing_trans_cap[tx] if period >= m.initial_build_year[tx] else 0)
    )
...
def load_inputs(mod, switch_data, inputs_dir):
    ...
    switch_data.load_aug(
        ...
        select=(
            ...
            'trans_efficiency', 'existing_trans_cap', 'trans_dbid',
            'trans_derating_factor', 'trans_terrain_multiplier',
            'trans_new_build_allowed', 'initial_build_year', 'trans_expand_after_year'
        ),
        index=mod.TRANSMISSION_LINES,
        optional_params=(
            'trans_dbid', 'trans_derating_factor', 'trans_terrain_multiplier',
            'trans_new_build_allowed', 'initial_build_year', 'trans_expand_after_year'
        ),
        param=(
            mod.trans_lz1, mod.trans_lz2, mod.trans_length_km, mod.trans_efficiency, 
            mod.existing_trans_cap, mod.trans_dbid, mod.trans_derating_factor, 
            mod.trans_terrain_multiplier, mod.trans_new_build_allowed, 
            mod.initial_build_year, mod.trans_expand_after_year
        )
    )
    switch_data.load_aug(
