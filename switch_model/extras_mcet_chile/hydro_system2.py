...
hydro_system2: this module will separate the flow in the case of various generators
in the same flow of water
...
def define_components(mod):
    ...
    #### Added module ####
    mod.GENS_IN_WATER_NODES = Set(
        mod.WATER_CONNECTIONS, 
        initialize = lambda m, wc: set(hg for hg in m.HYDRO_GENS
            if m.hydraulic_location[hg] == wc
        )
    )
    # mod.NODES_OF_HYDRO_GENS = set(
    #     mod.HYDRO_GENS, 
    #     initialize = lambda m, hg: set(wn for wn in m.WATER_NODES
    #         if m.hydraulic_location[hg] == wn
    #     )
    # )
    #####################
    ...
    mod.Enforce_Hydro_Extraction = Constraint(
        mod.WCON_TPS,
        rule=lambda m, wc, t: (sum(m.TurbinateFlow[g_in_node, t] +
            m.SpillFlow[g_in_node, t] for g_in_node in m.GENS_IN_WATER_NODES[wc]) <= 
            m.DispatchWater[wc, t]))
    ...
    # lambda m, wwy: set(g for g in m.HYDRO_GENS if m.hydro_gen_conn[g] == wwy)
    # {wwy:[gen1, gen2, ge...]}
